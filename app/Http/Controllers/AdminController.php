...
use function App\Common\Canh\getName;
...
//    }
    ...
    {
//            event(new Notify('okokokokok'));
            if($request->has('sort')||$request->has('field')){
                $user=DB::table('users')->orderBy($request->field,$request->sort)->paginate(config('constant.pagintion.page'))->appends(['sort'=>$request->sort,'field'=>$request->field]);
                if($request->sort=='asc'){
                    $request->sort='desc';
                }else{
                    $request->sort='asc';
                }
                return view('admin.manager_admin.index',compact('user','request'));
            }
    ...
    public function post($id, Request $request)
    {
        ...
        if($validtor->fails()){
            return redirect()->back()->withInput(Input::all())->withErrors($validtor->errors());
        }
        $user = User::findOrFail($id);
    ...
    public function search(Request $request)
    {
        ...
        return view('admin.manager_admin.index', ['user' => $users,'request'=>$request]);
